...
class TokenProvider {
  ...
  async getLatestToken() {
    ...
    let token = await this.getNewTokenOnce();
    this.setToken(token);
    return token;
  }
...
${xTime}`;
  ...
  _enforce(options, requiredKeys) {
    if (!options || typeof options !== 'object') {
      throw AppError.create(Messages.missing_parameter, 'options');
    }

    requiredKeys.forEach(function(requiredKey) {
      if (!options[requiredKey])
        throw AppError.create(Messages.missing_parameter, `options.${requiredKey}`);
    });
  }
}
